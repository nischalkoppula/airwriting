    ...
    if ret:
        ...
        # if not to take average
            ...
            if hand is not None:
                ...
                cv2.circle(c, (m[0], m[1]), 3, pointer_color, -3)
                cv2.circle(clone, (right+m[0], top+m[1]), 5, pointer_color, -3)
                
                if erase==True:
                    cv2.circle(canvas, (m[0], m[1]), 3, draw_color, -3)
                    erimg=cv2.circle(canvas.copy(), (m[0], m[1]), 3, (0, 0, 0), -3)            
                    cv2.circle(c, (m[0], m[1]), 3, (0, 0, 255), -3)
                    ...
                    erimg=cv2.circle(canvas.copy(), (m[0], m[1]), 3, (255, 255, 255), -3)            
                    cv2.circle(c, (m[0], m[1]), 3, (0, 0, 255), -3)
                    e = cv2.erode(erimg, (3, 3), iterations=5)
                    drawn = cv2.resize(e, (clone.shape[1], clone.shape[0]))                    
                elif erase==False:
                    ...
                    dc = drawn.copy()  
                    cv2.circle(dc, (m[0], m[1]), 5, pointer_color, -3)
